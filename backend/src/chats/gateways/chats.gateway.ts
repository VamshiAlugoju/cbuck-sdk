...
export class ChatsGateway extends BaseLogger {
    ...
    async handleSendMessage(client: Socket, payload: SendMessageDto) {
        ...
        const receiverSockerId = await getSocketId(payload.receiver);
        if (!receiverSockerId) {
            return {
                error: `Receiver with ID ${payload.receiver} is not connected.`,
            };
        }
        io.to(receiverSockerId).emit(chatEvents.RECEIVE_MESSAGE, payload);
        this.logger.log(`Message sent to ${payload.receiver}`);
    }
}
